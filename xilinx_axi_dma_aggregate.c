...
/* AXI AGGREGATE DMA Specific Registers/Offsets */
#define XILINX_DMA_DEVICE_OFFSET 0x10000
				  ...
				  struct device_node *node)
{
	...
	int device_id = 0;
	of_property_read_u32(node, "ict,device-id", &device_id);
	...
	if (of_device_is_compatible(node, "xlnx,axi-vdma-mm2s-channel") ||
		...
		chan->ctrl_offset = XILINX_DMA_MM2S_CTRL_OFFSET +
				    (device_id * XILINX_DMA_DEVICE_OFFSET);
...
static const struct xilinx_dma_config axidma_aggregate_config = {
	.dmatype = XDMA_TYPE_AXIDMA,
	.clk_init = axidma_clk_init,
	.irq_handler = xilinx_dma_irq_handler,
	.max_channels = XILINX_DMA_MAX_CHANS_PER_DEVICE + 0x2,
};
